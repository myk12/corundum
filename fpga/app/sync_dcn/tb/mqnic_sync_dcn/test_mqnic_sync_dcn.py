...
class SyncDCN:
    ...
    def start(self):
        for node in self.sw_nodes:
            cocotb.start_soon(node._run_consensus_app())  # Start software node runner
...
async def test_sync_consensus_1hw2sw(dut):
    ...
    sw_node1 = SoftwareNode(node_id=1, node_mac="00:11:22:33:44:55", ptp_clock_tod=ptp_clock)
    sw_node2 = SoftwareNode(node_id=2, node_mac="00:11:22:33:44:66", ptp_clock_tod=ptp_clock)
    
    # Test Consensus Normal Operation
    network = SyncDCN()
    tb.install_network(network)
    sw_node1.install_network(network)
    sw_node2.install_network(network)
    network.add_hw_dut(tb)
    network.add_sw_node(sw_node1)
    network.add_sw_node(sw_node2)

    tb.log.info("Test Consensus App: Normal Operation")
    network.start()

    await Timer(10, 'ms')  # Run the test for a certain duration
