...
class GpuNode:
    ...
    def display_packet(self,pkt):
        ...
        if eth_type == ETHTYPE_CONSENSUS:
            self.counter_consensus -= 1
        elif eth_type == ETHTYPE_BEFORE_ATTENTION:
            self.counter_before_atten -= 1
    ...
    async def check_status(self):
        if self.counter_consensus == 0 and self.send_counter == 0:
                self.log.info(f"Consensus Success - Node ID: {self.node_id}")
                self.counter_consensus = -1
                if self.state == PacketType.CONSENSUS_ATTENTION:
                    self.layer += 1
                    ...
                    self.counter_before_moe = 7
                    self.state = PacketType.BEFORE_MOE
                elif self.state == PacketType.CONSENSUS_MOE:
                    self.counter_before_atten = 7
                    self.state = PacketType.BEFORE_ATTENTION
                else:
                    assert False, f"Unexpected state {self.state} when consensus success at Node {self.node_id}"
        ...
        if self.counter_after_moe == 0 and self.send_counter == 0:
                ...
                self.state = PacketType.CONSENSUS_MOE
    ...
    async def _run_consensus_app(self):
        ...
        while self._running:
            ...
            if current_slot_id != last_slot_id and self.send_counter != 0:  # New slot and ready to send
                ...
                elif self.state in (PacketType.CONSENSUS_ATTENTION, PacketType.CONSENSUS_MOE):
                        pkt = self.create_packet(current_slot_id, ETHTYPE_CONSENSUS)
                else:
