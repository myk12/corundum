# Synchronous Consensus App Testbench
# This testbench verifies the functionality of the synchronous consensus application
# by performing AXILite read/write operations and ensuring proper packet exchange. 
...
try:
    ...
    # attempt to import from current directory
...
def display_consensus_packet(pkt):
    ...
    print(f"Consensus Packet - Src MAC: {src_mac}, Dst MAC: {dst_mac}, Ethertype: {eth_type:#06x}, Slot ID: {slot_id}, Node ID: {node_id}, Ack Vec: {ack_vec:#04x}")

class HardwareDut(object):
    def __init__(self, dut, node_id, msix_count=32):
        self.dut = dut
        self.app_block = dut.core_pcie_inst.core_inst.app.app_block_inst
        self.node_id = node_id
        self.network_connected = False

        self.log = SimLog("cocotb.tb_hw_dut")
    ...
    async def recv_packet(self, pkt: Ether):
        # Process received packet and interact with software nodes as needed
        self.log.info(f"Hardware DUT received packet: {pkt.summary()}")

        display_consensus_packet(pkt)
        # convert packet to raw bytes and send to hardware DUT's RX interface
        await RisingEdge(self.dut.clk)
        for mac in self.port_mac:
            await mac.rx.send(bytes(pkt))
    ...
    async def _run_consensus_app(self):
        ...
        self.loopback_enable = False  # Disable loopback to allow packet exchange with software nodes
        self.app_block.reg_ctrl.setimmediatevalue(1)
        ...
        while True:
            ...
            for mac in self.port_mac:
                if not mac.tx.empty():
                    pkt = await mac.tx.recv()
                    pkt = Ether(bytes(pkt))

                    display_consensus_packet(pkt)
                    # Process received packet and interact with software nodes as needed
                    self.network.synchronous_broadcast(pkt, sender_id=self.node_id, hw_node=True)
...
async def test_sync_consensus_1hw2sw(dut):
    """Test synchronous consensus application with 1 hardware and 2 software nodes"""
    tb = HardwareDut(dut, node_id=0, msix_count=2**len(dut.core_pcie_inst.irq_index))
    await tb.init()
    ...
    ptp_clock = dut.core_pcie_inst.core_inst.ptp_sync_ts_tod if dut.core_pcie_inst.core_inst.PTP_TS_FMT_TOD.value else dut.core_pcie_inst.core_inst.ptp_sync_ts_rel

    gpu_node1 = GpuNode(node_id=1, node_mac="00:11:22:33:44:55", ptp_clock_tod=ptp_clock)
    gpu_node2 = GpuNode(node_id=2, node_mac="00:11:22:33:44:56", ptp_clock_tod=ptp_clock)
    gpu_node3= GpuNode(node_id=3, node_mac="00:11:22:33:44:57", ptp_clock_tod=ptp_clock)
    gpu_node4= GpuNode(node_id=4, node_mac="00:11:22:33:44:58", ptp_clock_tod=ptp_clock)
    gpu_node5= GpuNode(node_id=5, node_mac="00:11:22:33:44:59", ptp_clock_tod=ptp_clock)
    gpu_node6= GpuNode(node_id=6, node_mac="00:11:22:33:44:5A", ptp_clock_tod=ptp_clock)
    gpu_node7= GpuNode(node_id=7, node_mac="00:11:22:33:44:5B", ptp_clock_tod=ptp_clock)
    gpu_node8= GpuNode(node_id=8, node_mac="00:11:22:33:44:5C", ptp_clock_tod=ptp_clock)

    
    # Test Consensus Normal Operation
    pipline1 = Switch([1,2,3,4], [5,6,7,8], switch_id=1)  # Initialize switch with 4 input ports and 4 output ports
    pipline2 = Switch([9,10,11,12], [13,14,15,16], switch_id=2)  # Initialize switch with 4 input ports and 4 output ports
    pipline3 = Switch([17,18,19,20], [21,22,23,24], switch_id=3)  # Initialize switch with 4 input ports and 4 output ports

    gpu_node1.install_network(pipline1, port=2)#不需要知道端口,接受就行就行
    gpu_node2.install_network(pipline1, port=4)
    gpu_node3.install_network(pipline1, port=6)
    gpu_node4.install_network(pipline1, port=8)
    #pipline1.add_hw_dut(tb)
    pipline1.add_sw_node(gpu_node1,2)
    pipline1.add_sw_node(gpu_node2,4)
    pipline1.add_sw_node(gpu_node3,6)
    pipline1.add_sw_node(gpu_node4,8)
    #初始化pipline2
    gpu_node5.install_network(pipline2, port=10)
    gpu_node6.install_network(pipline2, port=12)
    gpu_node7.install_network(pipline2, port=14)
    gpu_node8.install_network(pipline2, port=16)
    pipline2.add_sw_node(gpu_node5,10)
    pipline2.add_sw_node(gpu_node6,12)
    pipline2.add_sw_node(gpu_node7,14)
    pipline2.add_sw_node(gpu_node8,16)
    pipline1.add_switch(pipline3,[{"local": 1, "remote": 17},{"local": 3, "remote": 21},{'local': 5, 'remote': 18},{'local': 7, 'remote': 22}])
    pipline2.add_switch(pipline3,[{"local": 9, "remote": 19},{"local": 11, "remote": 23},{'local': 13, 'remote': 20},{'local': 15, 'remote': 24}])
    pipline3.add_switch(pipline1,[{"local": 17, "remote": 1},{"local": 21, "remote": 3},{'local': 18, 'remote': 5},{'local': 22, 'remote': 7}])
    pipline3.add_switch(pipline2,[{"local": 19, "remote": 9},{"local": 23, "remote": 11},{'local': 20, 'remote': 13},{'local': 24, 'remote': 15}])
    

    # tb.log.info("Test Consensus App: Normal Operation")
    pipline1.start()
    pipline2.start()
    pipline3.start()
