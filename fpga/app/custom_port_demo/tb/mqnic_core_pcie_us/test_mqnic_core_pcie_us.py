...
class TB(object):
    def __init__(self, dut, msix_count=32):
        ...
        self.toplevel_ram = AxiLiteRam(AxiLiteBus.from_prefix(dut, "m_axil_app_ctrl"), dut.clk, dut.rst, size=256*1024)
...
async def run_test_nic(dut):
    ...
    val = tb.toplevel_ram.read_dword(0)
    tb.log.info("Read data: 0x%08x", val)
    assert val == 0x11223344
...
def test_mqnic_core_pcie_us(request, if_count, ports_per_if, axis_pcie_data_width,
    ...
    verilog_sources = [
        ...
        os.path.join(rtl_dir, "mqnic_app_block_custom_port_demo.v"),
